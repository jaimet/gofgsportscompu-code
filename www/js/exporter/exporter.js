...
exporter.base = function() {
	this.self = this;
}

exporter.base.prototype.m_trackFileEntry = null;
exporter.base.prototype.m_completeCallback = null;
exporter.base.prototype.m_continuousWriter = null;
exporter.base.prototype.m_xmlDoc = null;
exporter.base.prototype.m_trackStartTime = null;
exporter.base.prototype.m_trackTotalDistance = null;

exporter.base.prototype.run = function( p_fileEntry, p_completeCallback ) {
	self.m_trackFileEntry = p_fileEntry;
	self.m_completeCallback = p_completeCallback;
	
	// Reset the members variables
	self.m_continuousWriter = null;
	self.m_xmlDoc = null;
	self.m_activityNode = null;
	self.m_trackNode = null;
	self.m_trackStartTime = 0;
	self.m_trackTotalDistance = 0;

	// Create basic XML structure
//	self.m_xmlDoc = $.parseXML( "<FitnessWorkbook xmlns='http://www.zonefivesoftware.com/xmlschemas/FitnessLogbook/v2' xmlns:xsi='http://www.w3.org/2001/XMLSchema-instance' xmlns:xsd='http://www.w3.org/2001/XMLSchema'></FitnessWorkbook>" );
//	var fwNode = self.m_xmlDoc.firstChild;
//	var alNode = self.m_xmlDoc.createElement( 'AthleteLog' );
//	fwNode.appendChild( alNode );
//	self.m_activityNode = self.m_xmlDoc.createElement( 'Activity' );
//	alNode.appendChild( self.m_activityNode );
//	self.m_trackNode = self.m_xmlDoc.createElement( 'Track' );
//	self.m_activityNode.appendChild( self.m_trackNode );
	
	// Construct new file-name
	var fileName = self.m_trackFileEntry.name;
	exportFileName = fileName.replace( '.gsc', '.fitlog' );

	// Get our export file
	// TODO: Remove external reference to TrackHandler
	TrackHandler.getExportDirectory().getFile( exportFileName, { create : true, exclusive : false }, function( p_fileEntry ) { self._fileEntry.call(self, p_fileEntry) }, GOFGSportsComputer._fileSystemError );
};

exporter.base.prototype._fileEntry = function( p_fileEntry ) {
	self.m_continuousWriter = new ContinuousFileWriter( p_fileEntry );
	self.m_continuousWriter.writeLine( "<?xml version='1.0' encoding='UTF-8' ?>" );

	new TrackReader( self.m_trackFileEntry, function( p_trackWaypoint ) { self._waypoint.call( self, p_trackWaypoint ) }, self._trackComplete );
}

exporter.base.prototype._waypoint = function( p_trackWaypoint ) {
