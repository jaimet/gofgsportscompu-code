...
exporter.gpx = {
	...
	run : function( p_fileEntry, p_completeCallback ) {
		...
		TrackHandler.getExportDirectory().getFile( exportFileName, { create : true, exclusive : false }, exporter.gpx._fileEntry, GOFGSportsComputer._fileSystemError );
	},
	...
	_waypoint : function( p_trackWaypoint ) {
		if( exporter.gpx.m_trackStartTime == 0 ) {
			exporter.gpx.m_trackStartTime = p_trackWaypoint.timestamp;
...
//			exporter.fitlog.m_activityNode.setAttribute( 'StartTime', formattedDate );
		...
		exporter.gpx.m_trackTotalDistance += p_trackWaypoint.distance;
	...
	_toString : function() {
		...
		try {
			...
			return (new XMLSerializer()).serializeToString(exporter.gpx.m_xmlDoc);
		}
		catch (e) {
			try {
				// Internet Explorer.
				return exporter.gpx.m_xmlDoc.xml;
			}
			catch (e) {
